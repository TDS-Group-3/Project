...
model_linear_male_lasso <- `model_linear_male_lasso-2`
my_data <- model_linear_male_lasso
...
df$group_name[df$group_name == "NULL"]  <- "Male Specific"
...
forest %>% 
  forestplot(labeltext = c(Group, Variable, Level, Coefficient_95_CI, P_value),
             ...
             lwd.ci = 3,
             boxsize = 0.2,
             title = 'Male-specific LASSO Model of Stably-selected Features against LTL',
             graph.pos = 5,
