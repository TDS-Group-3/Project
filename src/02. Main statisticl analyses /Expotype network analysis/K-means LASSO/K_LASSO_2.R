...
set.seed(1)
  ...
  stab=VariableSelection(xdata=x, ydata= ylist[[2]], verbose=FALSE,
                         K = 1000,
                         n_cores = 4,
                         family="binomial")
  t1=Sys.time()
  ...
  save(stab, file = paste0("VS_",2,".Rdata"))
  
  # append function to add to selection proportions
  
  
  pdf(file= paste0("CalibrationPlot_",2,".pdf"))
  ...
  saveRDS(selprop, file = paste0("logistic_LASSO_selection_proportions_",2,".rds"))
  ...
  saveRDS(LASSO_parameters, file = paste0("logistic_LASSO_parameters_",2,".rds"))
  
  # betas
  
  beta_matrix = stab$Beta
  
  save(beta_matrix, file = paste0("logistic_LASSO_betas_",2,".Rdata"))
  ...
  saveRDS(LASSO_selected_variables, file = paste0("logistic_LASSO_selected_variables_",2,".rds"))
